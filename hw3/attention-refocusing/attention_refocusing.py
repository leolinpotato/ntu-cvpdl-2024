...
def run(meta,models,info_files, p, starting_noise=None,iter_id=0, img_id=0, save=True):
    ...
    if save:
        ...
        for image_id, sample in zip(image_ids, samples_fake):
            ...
            save_img(output_folder2, sample, meta['image_name'], iter_id, img_id)
            save_img(output_folder1, img2, meta['image_name'], iter_id , img_id )
    return samples_fake
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", default="gligen_checkpoints/diffusion_pytorch_model.bin", type=str, required=True)
    ...
    path = os.path.join( args.folder, args.output_dir+'_img')
    if os.path.exists(path):
        file_cnt = count_files_in_dir(path)
    else:
        file_cnt = 0
    ...
    for i, d in enumerate(data):
        ...
        for meta in meta_list:
            ...
            for k in range(1):
                ...
                draw_box_2(o_names, box_att , layout_folder, meta['image_name'])
